    ...
    private:
            ...
            bool adding = true)
        {
            ...
            for(int layer = 0; valid && layer < nodeHeight; ++layer)
            {
                ...
                if(adding)
                { // when adding a node, the succ shouldn't be going away
        ...
        std::pair<NodeType*, int> findNodeRightDown(const value_type& data) const
        {
            ...
            auto top = maxLayer();
            int found = 0;
            for(int layer = top; (found == 0) && layer >= 0; --layer)
            {
                node = pred->skip(layer);
                while(greater(data, node))
                {
    ...
    public:
        ...
        csl_iterator(
            ...
            typename std::enable_if<
                std::is_convertible<OtherVal*, ValT*>::value>::type* /* */
            = nullptr)
            : node_(other.node_)
        {
        }
